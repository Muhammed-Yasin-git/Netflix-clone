...
function SavedShows() {
  ...
  useEffect(() => {
    if (user?.email) {
      const unsubscribe = onSnapshot(doc(db, 'netflix-users', user.email), (doc) => {
  ...
  const movieRef = doc(db,'netflix-users',`${user?.email}`)
          ...
          {movies.map((item, id) => (
            <div key={id} className='w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block cursor-pointer relative p-2'>
              ...
              <div className='absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 hover:opacity-100 text-white'>
                ...
                <p onClick={()=> deleteSaved(item.id)} className='absolute text-gray-400 top-4 right-4'><AiOutlineClose/></p>
              </div>
            </div>
          ))}
        </div>
